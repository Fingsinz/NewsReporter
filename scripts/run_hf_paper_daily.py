...
## 每个主题小节结构
1. **概述**（2-4 句）：说明该主题下发生了什么、涉及哪些方法/模型/数据集/机构，以事实陈述为主。
2. **分析**（2-4 句）：提炼技术意义、发展趋势或潜在影响，须基于概述内容推导，不得引入外部臆测。
...
## 来源标注
- 使用 Markdown 链接语法 `[关键词](URL)` 在正文中 **对应语句处** 自然嵌入来源。
- **禁止** 在文末集中列出参考来源。
- 同一来源在同一小节中仅需标注一次，避免重复链接。
